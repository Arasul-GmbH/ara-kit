...
With `/root` you lay out the root folder of a whole house, next to the kit and not inside it: rules with a table that says where the truth stands, `company/` with core, goal, decisions and follow-ups, a roadmap with one sheet per place and a card stack whose folder is the state, experiments, customers, templates, archive. The places where the work happens, GitHub repositories and foreign folders such as SharePoint, are named in a list and never copied, a hook keeps a session in the root from writing into them, and the rights stand per folder in `settings.json`. The root brings its own check script and runs with Node alone afterwards. `node .ara/tools/root.mjs --path <folder> --example` lays out an invented company as a showcase, and its own check script finds nothing in it.
