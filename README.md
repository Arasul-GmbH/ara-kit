...
With `/app` you build your own app and bring it onto a machine. Ara first asks what it is about, who uses it, what data goes in, where a language model does the work and at which point a human should decide; out of that comes a plan that you read before anything is built. The scaffold brings a frontend, a backend and a first flow, and it runs from the first minute: an item is filed, the flow stops at an approval, a human decides in Arasul, after that the item stands as approved or rejected, with the name of the one who decided. Building happens on your computer, deployment goes into the staging slot of the machine, you switch it live yourself. Who gets to see it there an administrator decides on the machine: an app is visible to a person only once it has been released for them, the kit's key may deploy and nothing else, and so after every deployment Ara says that the release is still missing and names the two ways to it, over the start password or in the machine's own interface. On a plain machine the same app runs over Compose, and the kit tells you what is missing there: login, flows, permissions.

With `/root` you lay out the root folder of a whole house, next to the kit and not inside it. Without a switch it is a scaffold: rules with a table that says where the truth stands, skills and agents, a list of the places where the work happens, the folders of level 1 that the house names, and its own check script that runs with Node alone. Places, GitHub repositories and foreign folders such as SharePoint, are named in the list and never copied. Nothing in the tree runs by itself: the boundary hook that keeps a session in the root from writing into a place and the permission rules are a proposal, and a separate step enrols them into your own settings after your consent, with a checksum. Company sheets, a roadmap with a card stack and experiments are the method, an addition you can choose with `--method` or lay in later. `node .ara/tools/root.mjs --path <folder> --example` lays out an invented company as a showcase, and its own check script finds nothing in it. With `--deploy` the root goes into its room on an Arasul device and lives there: whoever is given the room gets the root at the top of their own tree with the next sync of the bridge in the root.
...
With Arasul the machine itself becomes a source. `/device` installs the platform: it fetches the installer with your token from the portal, pushes it onto the machine, runs it there with a start password and a network name, and creates the key with which the kit later rolls apps onto it. What the installer is called the artifact says itself, the kit does not guess it; the start password lands in your secret store and nowhere else. That key you can also take back: `--keys` lists what lies on the machine and marks yours, `--revoke-key` revokes exactly that one and forgets it. What the platform can do beyond that, for instance creating the first employee and sharing something with them, is in the manuals that come with the artifact: `node .ara/tools/mirror.mjs --docs` tells you which those are, and `node .ara/tools/mirror.mjs --docs --device <machine>` reads them on the machine itself, without a token, also when somebody else installed it. After that Ara asks the machine what it promises instead of claiming something: manifest, limits and endpoints come out of its contract, and if the kit does not fit, she says so before anything is deployed. A token you only need for that installation: an account in the portal is free and brings one free token, it opens the download and nothing else. The installer hardens SSH so that only a key gets in afterwards, so before it runs the kit checks that login with a key works, and if not, it stops with one sentence instead of locking you out.
...
`node .ara/tools/selftest.mjs` checks, without network and without a machine, whether the kit works on your computer. The whole run takes minutes, measured on 25.09.2026 on a MacBook about seven, because one of its checks runs it a second time in a blank clone; `ARA_SELFTEST_ONLY=<pattern>` runs only the checks whose name matches, and that is no proof before a merge. Every tool answers `--help` with its header help and does nothing else while doing so. The documents are checked by `npx --yes markdownlint-cli2@0.18.1 --config .ara/.markdownlint-cli2.jsonc "**/*.md"`; the kit has no CI, both runs are the proof before a merge. `node .ara/tools/check-docs.mjs --device <device>` checks the other direction: it takes every route that stands in the kit's knowledge, holds it against your machine's contract and asks there. What no longer exists shows up before you work along it.
...
English is the kit's main language, German is equivalent and complete. `/init` asks in the first round, the answer goes into `business/profile.md` as `language: de|en`, and from then on Ara speaks that language and the tools print in it. Documents come in pairs: `README.md` and `.ara/README.de.md`, `.ara/knowledge/device.md` and `.ara/knowledge/device.de.md`, and so on for the commands and the scaffolds. The German README is the one pair that does not lie next to its English half: the root of the repository holds one README, the one GitHub shows. The self-test counts the pairs, so neither language can quietly fall behind.
