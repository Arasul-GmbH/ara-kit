...
for (const [name, value] of [
  ...
  if (value === true) fail(`--${name} braucht eine Angabe: --${name} <${name === "app" ? "name" : "wert"}>.`);
}
...
function createApp(name) {
  ...
  if (existsSync(dir)) {
    fail(`Die App ${name} gibt es schon: ${relative(ROOT, dir)}. Ansehen mit --app ${name}.`);
  }
  if (!existsSync(TEMPLATE)) fail(`Die Vorlage fehlt: ${relative(ROOT, TEMPLATE)}`);

  const titel = str(arg.titel) || name;
  const beschreibung = str(arg.beschreibung) || `${titel}, gebaut mit dem Ara-Kit.`;
...
function createPlan(app, titel) {
  if (!app.exists) fail(`Die App ${app.name} gibt es noch nicht. Zuerst --new.`);
  if (!existsSync(PLAN_TEMPLATE)) fail(`Die Planvorlage fehlt: ${relative(ROOT, PLAN_TEMPLATE)}`);
  const file = planFileName(titel);
  const dir = ensureDir(join(app.dir, "plans", "offen"));
  const path = join(dir, file);
  if (existsSync(path)) fail(`${relative(ROOT, path)} gibt es schon.`);
  writeFileSync(
    path,
  ...
  console.log(
    [
      `Plan angelegt: ${relative(ROOT, path)}`,
      "",
...
function shiftPlan(app, file, to) {
  if (!app.exists) fail(`Die App ${app.name} gibt es noch nicht.`);
  let moved;
  try {
...
function buildApp(app) {
  ...
  for (const entry of readdirSync(app.dir, { withFileTypes: true })) {
    ...
    if (existsSync(packageFile)) {
      try {
        ...
        fail(`${relative(ROOT, packageFile)} ist kein lesbares JSON: ${error.message}`);
      }
    }
    if (!script) {
      cpSync(source, target, {
    ...
    if (!existsSync(join(source, "node_modules"))) {
      console.log(`${entry.name}: die Abhängigkeiten fehlen, npm install ...`);
    ...
    if (built.status !== 0) fail(`Der Bau von ${entry.name} ist mit Rückgabecode ${built.status} beendet.`);
  ...
  console.log(
    [
      ...
      gebaut.length ? `Aus dem Quelltext gebaut: ${gebaut.join(", ")}.` : "Nichts zu bauen, alles lag fertig vor.",
      "",
      "Prüfen und einspielen geht nur gegen ein Gerät, dessen Kontrakt sagt, was gilt:",
      `  node .ara/tools/app.mjs --device <gerät> --app ${app.name} --check`,
    ].join("\n")
  );
}
...
function showApp(app) {
  ...
  if (!app.exists) {
    lines.push(`Unter apps/${app.name}/ liegt noch nichts.`);
  } else {
    lines.push(
      `- Ordner: ${relative(ROOT, app.dir)}`,
      app.manifest
      ...
      `- README: ${app.readme ? "vorhanden" : "fehlt"}`,
...
function folderFor(value) {
  ...
  const name = whichApp();
  if (!name) {
    fail("Sag, welche App: --app <name>, oder gib den Ordner an: --deploy <ordner>.");
  }
  const app = readApp(name);
  if (!app.exists) fail(`Die App ${name} gibt es noch nicht. Zuerst --new.`);
  if (!app.build.exists) {
    fail(
  ...
  if (app.build.stale) {
    fail(
      `Der Bau von ${name} ist älter als der Quelltext (${app.build.time}).\n` +
        `Noch einmal bauen: node .ara/tools/app.mjs --app ${name} --build`
    );
  }
  writeState({ app: name });
...
async function compose() {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) fail(`--port ${arg.port} ist keine Portnummer.`);

  let connection;
  try {
    connection = sshArgs(fields);
  } catch (error) {
    fail(`${error.message}\nNachsehen in ${relative(ROOT, device.file)}.`);
  }
  ...
  try {
    ...
    if (!shipped.ok) fail(`Die Dateien kamen nicht an: ${shipped.message || "kein Grund genannt"}`);
  } finally {
  ...
  console.log(
    [
      ...
      "",
      "Das ist ein Gerät ohne Arasul. Was hier fehlt:",
      ...WAS_FEHLT.map((satz) => `- ${satz}`),
      "",
      `Anhalten am Gerät: cd ${target} && docker compose down`,
    ].join("\n")
  );
}
