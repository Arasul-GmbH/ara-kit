...
1. **Prüf, ob ein geeigneter Rechner verfügbar ist.** Der Technikcheck aus dem Onboarding
   hält das in `mein/profil.md` fest.
...
2. **Prüfsumme vergleichen.**
   `node .ara/werkzeuge/datentraeger.mjs --pruefsumme <datei>`
...
4. **Vorschau.**
   `node .ara/werkzeuge/datentraeger.mjs --schreiben <abbild> --auf <kennung>`
   Zeigt, was passieren würde, ohne etwas zu tun. Das ist die Bestätigungsstufe.
5. **Schreiben.** Mit `--ja --ausfuehren`. Braucht Verwaltungsrechte; wenn das Passwort
