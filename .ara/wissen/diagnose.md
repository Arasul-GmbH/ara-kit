...
1. **Was genau passiert?** Nicht „geht nicht". Was hat wer gemacht, was hätte passieren
   ...
   lass es dir zeigen.
2. **Ist das Gerät erreichbar?** `node .ara/werkzeuge/geraet-finden.mjs --host <adresse>`,
   dann `node .ara/werkzeuge/fern.mjs --kunde <k> --pruefen`. Wenn nicht: Verfahren in
   `.ara/wissen/fernzugriff.md`, Abschnitt „Wenn ein Gerät nicht mehr erreichbar ist".
...
Was war, was du getan hast, was es gebracht hat — in `kunden/<k>/verlauf/`. Beim nächsten
