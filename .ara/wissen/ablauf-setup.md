...
```
node .ara/werkzeuge/laufzettel.mjs --kunde <k> --stand
```
...
```
node .ara/werkzeuge/laufzettel.mjs --anlegen --kunde <k> --geraet <g>
```

### Immer nach jedem Schritt: eintragen

```
node .ara/werkzeuge/laufzettel.mjs --kunde <k> --phase <n> --status <fertig|unterbrochen> \
  --eintrag "Was getan wurde. Nachweis: was du geprüft hast und was dabei herauskam."
```
...
- **Geräteakte anlegen**, falls es sie noch nicht gibt: `.ara/vorlagen/geraet.md` nach
  `kunden/<k>/geraete/<g>/geraet.md` kopieren und ausfüllen, soweit bekannt. Die
  Gerätebezeichnung benennt Standort oder Rolle, nicht das Modell.
- **Laufzettel anlegen:**
  `node .ara/werkzeuge/laufzettel.mjs --anlegen --kunde <k> --geraet <g>`
- Gerät bestimmen und Reifegrad prüfen (`.ara/wissen/geraete-bestimmen.md`). Ist das Profil
...
- Lizenztoken prüfen: `node .ara/werkzeuge/spiegel.mjs --status`. Ein abgelaufener Token
...
Verfahren steht in `.ara/wissen/boot-und-flash.md`. Kurz:
...
- Erreichbarkeit prüfen: `node .ara/werkzeuge/geraet-finden.mjs --host <adresse>`
- Ist nichts erreichbar: `--nachbarn` zeigt, was im Netz überhaupt antwortet.
...
- Adresse, Anmeldename, Port und Schlüsselname in `geraet.md` eintragen.
- Ab jetzt läuft jeder Befehl über `node .ara/werkzeuge/fern.mjs --kunde <k> --befehl "…"` —
  damit stimmt das Ziel immer und die Ausführung ist protokollierbar.

**Nachweis:** `node .ara/werkzeuge/fern.mjs --kunde <k> --pruefen` meldet, dass die
Verbindung steht.
...
- **Fernzugriff einrichten** (`.ara/wissen/fernzugriff.md`).

Ändert sich dabei der Zugangsport oder der Anmeldename, **sofort in `geraet.md` nachziehen**
— sonst greift das nächste `fern.mjs` ins Leere.
...
Die Prüfliste steht in `.ara/wissen/abnahme.md`. Kern:
...
- `abnahme.md` aus dem Laufzettel erzeugen (Vorlage: `.ara/vorlagen/abnahme.md`).
  Sie entsteht aus dem Protokoll — deshalb kann sie nicht veralten.
- Kurzanleitung für die Mitarbeiter (Vorlage: `.ara/vorlagen/kurzanleitung.md`). Eine
...
- Status in `kunde.md` und `geraet.md` auf den Betriebszustand setzen,
