...
Mit `/root` legst du den Wurzelordner eines ganzen Hauses an, neben dem Kit und nicht darin: Regeln mit einer Tabelle, die sagt, wo die Wahrheit steht, `company/` mit Kern, Ziel, Entscheidungen und Wiedervorlagen, eine Roadmap mit einem Blatt je Ort und einem Kartenstapel, dessen Ordner der Status ist, Experimente, Kunden, Vorlagen, Archiv. Die Orte, an denen gearbeitet wird, GitHub-Repositories und fremde Ordner wie SharePoint, stehen in einer Liste und werden nie kopiert, ein Hook hält eine Sitzung in der Wurzel davon ab, in sie zu schreiben, und die Rechte stehen je Ordner in `settings.json`. Die Wurzel bringt ihr eigenes Prüfskript mit und läuft danach mit Node allein. `node .ara/tools/root.mjs --path <ordner> --example` legt eine erfundene Firma als Vorzeigefassung aus, und ihr eigenes Prüfskript findet darin nichts.
