...
instruction says so. Every field carries a `title` the page shows, `ARTEN` says date or amount.
Whoever discards a reading triggers a new one: the log keeps every one, and for a document that
